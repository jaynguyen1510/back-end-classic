...
const createPayment = async (orderDetails) => {
    ...
    const embed_data = {
        redirecturl: `https://the-classic-clothing.vercel.app/zalopay-success?app_trans_id=${app_trans_id}`, // Sử dụng `transID` thay vì `order.app_trans_id` vì `order` chưa được khai báo
    };
    const order = { // Đảm bảo rằng `order` được khai báo và khởi tạo tại đây
        ...
        bank_code: "",
        callback_url: " https://back-end-classic-ib8i.onrender.com/api/zalopay/callback"
    };
